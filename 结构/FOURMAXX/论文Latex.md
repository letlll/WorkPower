...
[标题数字去除 1](../../效率/MDwriteTools/标题数字去除%201.md)
...
\subsection{2.3.1 高度变化系数 \( C_z \) 计算}
...
\subsection{2.3.2 内部压力系数 \( C_i \) 计算}

内部压力系数 \( C_i \) 取决于建筑物的开口情况及气流特性。假设本项目为封闭式结构，内部压力系数 \( C_i = 0.6 \)。

\subsection{2.3.3 地面粗糙度系数 \( C_g \) 计算}

根据建筑所在地的地面粗糙度分类，北京属于中粗糙地面，\( C_g = 0.8 \)。

\subsection{2.3.4 基本风压 \( q_b \) 确定}
